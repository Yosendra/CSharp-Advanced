...
//  -> AutoResetEvent allows one waiting thread to release meanwhile the others will keep waiting whereas ManualResetEvent allows all waiting thread to release 
//  -> ManualResetEvent need explicitly invoke .Reset() after invoke .Set() whereas AutoResetEvent automatically giving unsignaled after invoke .Set()
class Shared
{
    ...
    // Here we use AutoResetEvent instead of ManualResetEvent
    ...
    static Shared()
    {
        ...
        BatchCount = 5;
        BatchSize = 3;
        Event = new AutoResetEvent(false);    // We want it unsignaled by default at start up, so give it false
    }
}
...
class Producer
{
    public void Produce()
    {
        ...
        for (int i = 0; i < Shared.BatchCount; i++)     // 5 times
        {
            ...
            // AutoResetEvent allows one waiting thread to proceed and immediatly
            // reset event to "unsignaled" so explicitly reset is not required
            Shared.Event.Set();

            // Here we do not call .Reset() because we use AutoResetEvent
            // Shared.Event.Reset();
        }
